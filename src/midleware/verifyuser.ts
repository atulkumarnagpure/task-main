...
import { redisClient } from "../model/redisconection";
...
async function checkLimit(
    ...
    const [rows]: any = await mysqldb.query(
        "SELECT count, start_time FROM rate_limits WHERE rate_key = ?",
        [key]
    );

    if (rows?.length === 0) {
        await mysqldb.query(
            "INSERT INTO rate_limits (rate_key, count, start_time) VALUES (?, ?, ?)",
            [key, 1, now]
        );
        return true;
    }
    ...
    if (now - start_time > WINDOW_MS) {
        await mysqldb.query(
            "UPDATE rate_limits SET count = 1, start_time = ? WHERE rate_key = ?",
            [now, key]
        );
        return true;
    }
